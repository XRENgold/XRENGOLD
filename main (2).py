...
def get_main_keyboard():
    ...
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🔗 Реферальная ссылка"), KeyboardButton(text="📢 Каналы"), KeyboardButton(text="❓ Помощь")]
        ],
...
async def get_subscription_keyboard():
    """Создает клавиатуру со ссылками на каналы"""
    channels = db.get_channels()
...
async def subscription_required(message: types.Message):
    """Проверяет подписку и если не подписан - отправляет сообщение с кнопками"""
...
async def show_help(message: types.Message):
    ...
    text = (
        ...
        "💳 <b>Вывод</b> - вывод средств (нужен скриншот)\n"
    ...
    if task[4] == "subscription":
        try:
            ...
            if member.status != 'left':
                ...
                if success:
                    await callback.message.delete()
                    # Показываем обновленный список заданий
            ...
            logging.error(f"Ошибка проверки задания: {e}")
...
async def withdraw_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "📸 Теперь отправьте скриншот вашего объявления на рынке\n\n"
            "Это подтверждение того, что вы разместили товар",
            reply_markup=get_back_keyboard()
        )
...
async def withdraw_screenshot(message: types.Message, state: FSMContext, bot: Bot):
    # Получаем фото
    photo = message.photo[-1]  # Берем самое большое качество
    ...
    await message.answer(
        "✅ Скриншот сохранен!\n\n"
        "📝 Теперь введите паттерн/скин для вывода:",
        reply_markup=get_back_keyboard()
    )
...
async def withdraw_screenshot_invalid(message: types.Message):
    await message.answer(
        "❌ Пожалуйста, отправьте скриншот в виде изображения!",
        reply_markup=get_back_keyboard()
    )
                ...
                try:
                    ...
                    with open(data['screenshot_path'], 'rb') as photo:
                        await bot.send_photo(
                            admin_id,
                            photo,
                            caption=f"📸 Скриншот к заявке #{withdrawal_id}"
                        )
...
async def process_withdrawal(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.answer("❌ Не удалось загрузить скриншот")
